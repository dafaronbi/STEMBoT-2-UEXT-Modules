...
#include "greenLED.h"
    ...
    #define greenLED_SetP4PinDriveMode(shift, mode)  \
    do { \
        greenLED_PC =   (greenLED_PC & \
                                (uint32)(~(uint32)(greenLED_DRIVE_MODE_IND_MASK << \
                                (greenLED_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (greenLED_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define greenLED_SetP4PinDriveMode(shift, mode)  \
        do { \
            greenLED_USBIO_CTRL_REG = (greenLED_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(greenLED_DRIVE_MODE_IND_MASK << \
                                    (greenLED_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (greenLED_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		greenLED_SetP4PinDriveMode(greenLED__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(greenLED_DR & (uint8)(~greenLED_MASK));
    drVal = (drVal | ((uint8)(value << greenLED_SHIFT) & greenLED_MASK));
    greenLED_DR = (uint32)drVal;
}
...
{
    return (uint8)((greenLED_PS & greenLED_MASK) >> greenLED_SHIFT);
}
...
{
    return (uint8)((greenLED_DR & greenLED_MASK) >> greenLED_SHIFT);
}
...
void greenLED_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  greenLED_INTCFG & (uint32)(~(uint32)position);
    greenLED_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(greenLED_INTSTAT & greenLED_MASK);
	greenLED_INTSTAT = maskedStatus;
    return maskedStatus >> greenLED_SHIFT;
}
